...
const CreateTest = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "title") {
      setTitleError(null); // Reset title error when user starts typing
    }
  };
  ...
  const generateUniqueId = (() => {
    ...
    return () => {
      counter++;
      return Date.now() + counter; // Combine timestamp with a counter for uniqueness
    };
  })();
  ...
  const handleNextStep = () => {
    ...
    if (step === 1) {
      if (!testDetails.title.trim()) {
        setTitleError("Please provide a test title."); // Set title error
        return;
      }
    }
  ...
  const handleAddAIQuestions = (aiQuestions) => {
    console.log("Setting questions in state:", aiQuestions);
